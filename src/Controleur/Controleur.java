...
public class Controleur implements ContainerListener, ActionListener, ItemListener {
	...
	public static boolean verification(Integer[] coord, boolean deepsearch) {
		...
		if (coordVerif[2] == 0) {
			...
			while (coordVerif[0] - 1 >= 0 && !extremiteMot) {
				if (PlateauDeJeu.boutonTab[coordVerif[0] - 1][coordVerif[1]].getText().length() == 1 && !extremiteMot) {
					coordVerif[0] -= 1;
				} // Obtention de la position de départ
				else {
					extremiteMot = true;
		...
		if (!deepsearch) { // On ne parcourt pas tous les mots accolés si ce n'est pas le mot initial
			...
			while (coordVerif[1] <= 14 && !extremiteMot) {
				...
				if (PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().length() == 1) { // Tant que lettre
																									// posé

					if (!motjoueVerif.isEmpty()) {
						motjoueVerif
								.remove(PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().toLowerCase());
					}
					mot += PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().toLowerCase();
					...
					if (coordVerif[0] - 1 >= 0) {
						if (PlateauDeJeu.boutonTab[coordVerif[0] - 1][coordVerif[1]].getText().length() == 1) {
							correct = Controleur.verification(new Integer[] { coordVerif[0] - 1, coordVerif[1], 0 },
									false);
						}
					}
			...
			while (coordVerif[0] <= 14 && !extremiteMot) {
				...
				if (PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().length() == 1) { // Tant que lettre
					...
					if (coordVerif[1] - 1 >= 0) {
						if (PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1] - 1].getText().length() == 1) {
							correct = Controleur.verification(new Integer[] { coordVerif[0], coordVerif[1] - 1, 1 },
									false);
						}
					}
		...
		if (!Modele.DICTIONNAIRE.containsKey(mot) || !motjoueVerif.isEmpty()) {
			correct = false;
		}
	...
	public static Integer compterPoint(Integer[] coord, boolean deepsearch) {
		...
		if (PlateauDeJeu.motjoue.size() == 7 && point == 0) {
			point += 50;
		} // SCRABBLE!
			...
			while (coordVerif[1] <= 14 && !extremiteMot) {
				...
				if (PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().length() == 1) {
					if (Controleur.estDansLeTab(Vue.MD, coordVerif)) {
						coef += 1;
						mot += Modele.LETTRES
								.get(PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().charAt(0))[0];
						...
						mot += Modele.LETTRES
								.get(PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().charAt(0))[0] * 2;
					} else if (Controleur.estDansLeTab(Vue.LT, coordVerif)) {
						mot += Modele.LETTRES
								.get(PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().charAt(0))[0] * 3;
					} else {
						mot += Modele.LETTRES
								.get(PlateauDeJeu.boutonTab[coordVerif[0]][coordVerif[1]].getText().charAt(0))[0];
					}
					...
					if (coordVerif[0] - 1 >= 0) {
						if (PlateauDeJeu.boutonTab[coordVerif[0] - 1][coordVerif[1]].getText().length() == 1
								&& Controleur.estDansLeTab(PlateauDeJeu.coordPourAnnuler,
										new Integer[] { coordVerif[0], coordVerif[1] })
								&& deepsearch) {
							point += Controleur.compterPoint(new Integer[] { coordVerif[0] - 1, coordVerif[1], 1 },
									false);
						}
					...
					if (coordVerif[1] - 1 >= 0) {
						if (PlateauDeJeu.boutonTab[coordVerif[1] - 1][coordVerif[1]].getText().length() == 1
								&& Controleur.estDansLeTab(PlateauDeJeu.coordPourAnnuler,
										new Integer[] { coordVerif[0], coordVerif[1] })
								&& deepsearch) {
							point += Controleur.compterPoint(new Integer[] { coordVerif[1] - 1, coordVerif[1], 1 },
									false);
						}
	...
	public static boolean estDansLeTab(int[][] matrix, Integer[] tab) {
		...
		for (int i = 0; i < matrix.length; i++) {
			if (matrix[i][0] == tab[0] && matrix[i][1] == tab[1]) {
				return true;
			}
		}
		return false;
