	...
	class LettreListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (action == valider) {
				if (Controleur.verification(PlateauDeJeu.coord, true)) {
					...
					Vue.vueDuJeu.historique.ajoutHistorique(
							"Joueur " + Modele.tour + " a joué le mot : " + Modele.joueurs[Modele.tour].mot
			...
			if (action == passer) {
				...
				while (indicelettrechange.size() != nblettrechange) { // tant que la liste des indices de lettres à
																		// changer n'est pas égale au nombre de lettres
					...
					// Si le joueur entre 0 lettres à changer, soit c'est une erreur missclick, soit
					...
					if (nblettrechange == 0) {
						...
						if (retour == 0) { // Cas ou le joueur demande à passer son tour
							...
							indicelettrechange.add(0);// on ajoute un élément pour que la taille de la liste soit
														// différente de nblettrechange deja égal à zéro
							// pour retourner dans le while. Ajouter cet élément n'est pas gênant car la
							// liste est réinitialisée de toute manière
						}
						...
						// Double boucle qui parcourt l'arraylist des indices à la recherche de
						// doublons. Si un ou plusieurs doublons sont détectés
						...
						// Si indiceachanger a une valeur différente que celle du début de boucle, alors
						// un doublons a été détecté au-dessus donc on supprime
						// l'élément de l'arraylist pour qu'elle n'ait plus la bonne taille et qu'on
						// retourne dans le while
						if (indiceachanger != -1) {
							indicelettrechange.remove(indiceachanger);
						}

						// Ce if a la même condition que le while. Si cette condition est vérifiée, le
						...
						if (indicelettrechange.size() != nblettrechange) {
							...
							Vue.vueDuJeu.historique
									.ajoutHistorique("Le joueur " + Modele.tour + " a décidé de changer ses lettres !");
							Controleur.passer();
							Vue.vueDuJeu.historique.ajoutHistorique("C'est à joueur " + Modele.tour + " de jouer !");
						}
					}
